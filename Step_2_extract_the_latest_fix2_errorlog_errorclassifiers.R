...
# Function to classify error severity
classify_error_severity <- function(error_type, additional_info = "") {
  # Level 0: No exception
  # Level 1: Minor issues (formatting, small mismatches, missing optional data)
  # Level 2: Moderate issues (data inconsistencies that can be handled, duplicates)
  # Level 3: Severe issues (failed operations, missing critical data)
  
  if(grepl("cleaned up filename|missing.*column|mismatch.*5|shorter.*5|longer.*5", error_type, ignore.case = TRUE)) {
    return(1)  # Minor
  } else if(grepl("duplicate|averaging|unexpected gas|missing gas|timing mismatch|inconsistent", error_type, ignore.case = TRUE)) {
    return(2)  # Moderate  
  } else if(grepl("failed|empty|error", error_type, ignore.case = TRUE)) {
    return(3)  # Severe
  } else {
...
# Function to synthesize error messages
synthesize_error <- function(errors) {
  ...
  if(any(grepl("failed.*download|failed.*series|failed.*flux", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "API_FAILURE")
  }
  if(any(grepl("empty", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "EMPTY_DATA")
  }
  if(any(grepl("mismatch|shorter|longer", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "LENGTH_MISMATCH")
  }
  if(any(grepl("duplicate|averaging", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "DUPLICATE_DATA")
  }
  if(any(grepl("timing|inconsistent", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "TIMING_ISSUE")
  }
  if(any(grepl("filename", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "FILENAME_CLEANED")
  }
  if(any(grepl("missing.*column", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "MISSING_COLUMNS")
  }
  
  return(paste(error_summary, collapse = "; "))
}

# Enhanced error logging function that also tracks per-subseries errors
subseries_errors <- list()  # Global list to track errors per subseries
...
log_error <- function(message, measurement_id = NA, series_id = NA, site_id = NA, subsiteid = NA, point = NA, gas = NA, additional_info = "") {
  ...
  # Track error for specific subseries if series_id is provided
  if(!is.na(series_id)) {
    key <- as.character(series_id)
    if(is.null(subseries_errors[[key]])) {
      subseries_errors[[key]] <<- character()
    }
    subseries_errors[[key]] <<- c(subseries_errors[[key]], message)
  }
}
...
for(i in 1:length(meas_ids)){
  ...
  for(site_name in sites_in_series) {
    ...
    tryCatch({
      if(file.exists(file.path(datafile_path, paste0(site_name, "whole.csv")))) {
        ...
        # Add missing exception columns if they don't exist
        if(!"exception_severity" %in% names(site_datafile)) {
          site_datafile$exception_severity <- 0L
        }
        
        if(!"exception_messages" %in% names(site_datafile)) {
          site_datafile$exception_messages <- ""
        }
        
        # Add missing db_origin column if it doesn't exist
        if(!"db_origin" %in% names(site_datafile)) {
          site_datafile$db_origin <- 3L
        }
        ...
        site_datafile <- tibble(
          ...
          trimmer=character(), 
          filename=character(),
          exception_severity=integer(),
          exception_messages=character(),
          db_origin=integer()
        )
        ...
        if(file.exists(paste(datafile_path_file, "db3like.csv", sep=""))) {
          ...
          # Add exception columns to old data if missing
          if(!"exception_severity" %in% names(old_datafile)) {
            old_datafile$exception_severity <- 0L
          }
          if(!"exception_messages" %in% names(old_datafile)) {
            old_datafile$exception_messages <- ""
          }
          if(!"db_origin" %in% names(old_datafile)) {
            old_datafile$db_origin <- 3L
          }
      ...
      site_datafile <- tibble(
        ...
        filename=character(), exception_severity=integer(), exception_messages=character(),
        db_origin=integer()
      )
    })
  ...
  for(j in 1:total_subseries){
    ...
    for(k in which(sites_in_series == current_site)){
      ...
      # Calculate exception severity and messages for this subseries
      current_errors <- subseries_errors[[as.character(s_id)]]
