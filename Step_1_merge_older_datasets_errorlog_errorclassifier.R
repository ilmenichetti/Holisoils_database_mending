...
# ----------------------------------------------------------------------------------------
# Initialize error logging (compatible with step 2)
# ----------------------------------------------------------------------------------------

# Function to classify error severity (same as API script)
classify_error_severity <- function(error_type, additional_info = "") {
  # Level 0: No exception
  # Level 1: Minor issues (normal flux found, small inconsistencies)
  # Level 2: Moderate issues (multiple flux values that can be handled)
  # Level 3: Severe issues (missing critical flux data)
  
  if(grepl("flux found|value found", error_type, ignore.case = TRUE)) {
    return(1)  # Minor - normal operation, flux data available
  } else if(grepl("multiple.*flux|many.*flux", error_type, ignore.case = TRUE)) {
    return(2)  # Moderate - data inconsistencies that can be handled
  } else if(grepl("no.*flux|missing.*flux", error_type, ignore.case = TRUE)) {
    return(3)  # Severe - missing critical data
  } else {
    return(1)  # Default to minor for unclassified
  }
}

# Function to synthesize error messages (same as API script)
synthesize_error <- function(errors) {
  if(length(errors) == 0) return("")
  
  # Group similar errors and create concise summary
  error_summary <- character()
  
  if(any(grepl("no.*co2|missing.*co2", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "MISSING_CO2")
  }
  if(any(grepl("no.*ch4|missing.*ch4", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "MISSING_CH4")
  }
  if(any(grepl("no.*n2o|missing.*n2o", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "MISSING_N2O")
  }
  if(any(grepl("multiple.*co2|many.*co2", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "MULTIPLE_CO2")
  }
  if(any(grepl("multiple.*ch4|many.*ch4", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "MULTIPLE_CH4")
  }
  if(any(grepl("multiple.*n2o|many.*n2o", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "MULTIPLE_N2O")
  }
  if(any(grepl("co2.*found", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "CO2_FOUND")
  }
  if(any(grepl("ch4.*found", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "CH4_FOUND")
  }
  if(any(grepl("n2o.*found", errors, ignore.case = TRUE))) {
    error_summary <- c(error_summary, "N2O_FOUND")
  }
  
  return(paste(error_summary, collapse = "; "))
}

#  error logging function that tracks per-series errors 
series_errors <- list()  # Global list to track errors per series

log_db_error <- function(message, series_id = NA, gas_type = NA, db_origin = NA, folder_name = NA) {
  # Create identifier for this series
  if(!is.na(series_id)) {
    key <- paste(series_id, gas_type, db_origin, sep="_")
    if(is.null(series_errors[[key]])) {
      series_errors[[key]] <<- character()
    }
    series_errors[[key]] <<- c(series_errors[[key]], message)
  }
  
  # Also print to console for monitoring
  cat("DB Error:", message, "Series:", series_id, "Gas:", gas_type, "DB:", db_origin, "\n")
}
...
#should be fixed now, commented out
...
# Initialize error tracking with consistent data types (keep original for compatibility)
errors_list <- list()
error_counter <- 0

# Helper function to add errors consistently (keep original for compatibility)
...
for (folder in site_ids) {
  ...
  #  Directory structure - align with Step 2 naming convention
  ...
  # Treatment info is contained in subsiteid, point, pointtype, this field is kept for compatibility
  series_subset$treatment <- NA
  
  # Process each series
  for (i in seq_len(nrow(series_subset))) {
    series_id <- series_subset$id[i]
    #  Directory structure - align with Step 2
    ...
    # Create filename including timing to avoid collisions
    ...
    if (nrow(file_to_write) > 0) {
      # Feather file structure - standardize column names for compatibility with Step 2 
    ...
    if (nrow(co2_fluxes) == 1) {
      ...
      log_db_error(paste("CO2 flux found, series", series_id), series_id, "co2", series_subset$db_origin[i], folder)
      ...
      add_error(no_co2 = TRUE, error_msg = paste("No CO2 flux found for series", series_id),
                flux_id = series_id, meas_set = subfolder,
                db_orig = series_subset$db_origin[i], folder_name = folder)
      log_db_error(paste("No CO2 flux found for series", series_id), series_id, "co2", series_subset$db_origin[i], folder)
    } else {
      ...
      add_error(many_co2 = TRUE, error_msg = paste("Multiple CO2 fluxes found for series", series_id, "- using first"),
                flux_id = series_id, meas_set = subfolder,
                db_orig = series_subset$db_origin[i], folder_name = folder)
      log_db_error(paste("Multiple CO2 fluxes found for series", series_id, "- using first"), series_id, "co2", series_subset$db_origin[i], folder)
    }
    ...
    if (nrow(ch4_fluxes) == 1) {
      ...
      add_error(ch4_val = TRUE, error_msg = paste("CH4 flux found, series", series_id),
                flux_id = series_id, meas_set = subfolder,
                db_orig = series_subset$db_origin[i], folder_name = folder)
      log_db_error(paste("CH4 flux found, series", series_id), series_id, "ch4", series_subset$db_origin[i], folder)
      ...
      add_error(no_ch4 = TRUE, error_msg = paste("No CH4 flux found for series", series_id),
                flux_id = series_id, meas_set = subfolder,
                db_orig = series_subset$db_origin[i], folder_name = folder)
      log_db_error(paste("No CH4 flux found for series", series_id), series_id, "ch4", series_subset$db_origin[i], folder)
    } else {
      ...
      add_error(many_ch4 = TRUE, error_msg = paste("Multiple CH4 fluxes found for series", series_id, "- using first"),
                flux_id = series_id, meas_set = subfolder,
                db_orig = series_subset$db_origin[i], folder_name = folder)
      log_db_error(paste("Multiple CH4 fluxes found for series", series_id, "- using first"), series_id, "ch4", series_subset$db_origin[i], folder)
    }
    ...
    if (nrow(n2o_fluxes) == 1) {
      ...
      add_error(n2o_val = TRUE, error_msg = paste("N2O flux found, series", series_id),
                flux_id = series_id, meas_set = subfolder,
                db_orig = series_subset$db_origin[i], folder_name = folder)
      log_db_error(paste("N2O flux found, series", series_id), series_id, "n2o", series_subset$db_origin[i], folder)
      ...
      add_error(no_n2o = TRUE, error_msg = paste("No N2O flux found for series", series_id),
                flux_id = series_id, meas_set = subfolder,
                db_orig = series_subset$db_origin[i], folder_name = folder)
      log_db_error(paste("No N2O flux found for series", series_id), series_id, "n2o", series_subset$db_origin[i], folder)
    } else {
      ...
      add_error(many_n2o = TRUE, error_msg = paste("Multiple N2O fluxes found for series", series_id, "- using first"),
                flux_id = series_id, meas_set = subfolder,
                db_orig = series_subset$db_origin[i], folder_name = folder)
      log_db_error(paste("Multiple N2O fluxes found for series", series_id, "- using first"), series_id, "n2o", series_subset$db_origin[i], folder)
    }
  }
  ...
  for (i in seq_len(nrow(series_subset_long))) {
    ...
    # Calculate exception severity and messages for this specific series/gas combination
    error_key <- paste(series_id, gas_type, db_origin, sep="_")
    current_errors <- series_errors[[error_key]]
    if(is.null(current_errors)) current_errors <- character()
    
    exception_severity <- if(length(current_errors) == 0) {
      0
    } else {
      max(sapply(current_errors, classify_error_severity))
    }
    
    exception_messages <- synthesize_error(current_errors)
    ...
    # Create final row (COMPATIBLE with API script output)
    final_data_list[[i]] <- data.frame(
      ...
      exception_severity = exception_severity,  # NEW: Compatible with API script
      exception_messages = exception_messages,  # NEW: Compatible with API script
      db_origin = row$db_origin,
      stringsAsFactors = FALSE
    )
  }
...
# Combine errors consistently (keep original error format for compatibility)
