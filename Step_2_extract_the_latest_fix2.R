...
for(i in 1:length(meas_ids)){
  ...
  for(group_name in names(grouped_data)) {
    ...
    # Extract gas types and check for consistency
    ...
    if(length(gas_types) != length(unique_gases)) {
      cat("    INFO: Duplicate gas types detected - will average duplicate measurements\n")
      for(gas in unique_gases) {
        duplicates <- which(gas_types == gas)
        if(length(duplicates) > 1) {
          cat(paste("      Gas", gas, "has", length(duplicates), "measurements - will average them\n"))
          for(dup_i in duplicates) {
            s_obj <- group[[dup_i]]$s_obj
            cat(paste("        Series", s_obj$id, ": measurement_id=", s_obj$measurements, "\n"))
          }
        }
      }
    }
    ...
    if(length(unexpected) > 0) {
      stop(paste("Unexpected gas types found:", paste(unexpected, collapse=", ")))
    }
    
    # Check maximum gas limit (should be max 3 gases per measurement session)
    if(length(unique_gases) > 3) {
      stop(paste("Too many gas types in group", group_name, "- maximum 3 expected, found:", length(unique_gases)))
    }
    ...
    for(gas in unique_gases) {
      ...
      if(length(unique(gas_instances)) > 1) {
        stop(paste("Inconsistent gas naming for", gas, ":", paste(unique(gas_instances), collapse=", ")))
      }
    }
    ...
    if(length(group) > 1) {
      ...
      for(j in 2:length(group)) {
        curr <- group[[j]]$s_obj
        curr_time_seq <- group[[j]]$time_sequence_formatted
        ...
        if(curr$date != reference_subseries$date || 
          ...
          stop(paste("Timing mismatch in group", group_name, "- grouping logic error"))
        }
        
        # Verify time sequence consistency
        if(length(curr_time_seq) != length(reference_time_seq)) {
          stop(paste("Time sequence length mismatch in group", group_name))
        }
        
        if(!identical(curr_time_seq, reference_time_seq)) {
          stop(paste("Time sequence values differ in group", group_name))
        }
      }
    }
    
    # Create combined feather file structure
    ...
    # The casewas  measurement_id 360, series 16906 & 17017 (both CO2 at 09:30:53-09:32:22)
    ...
    for(gas in unique_gases) {
      ...
      if(length(gas_indices) == 1) {
        ...
        if(length(values) == 0) {
          cat(paste("    Empty measurement series for", gas, "in group", group_name, "\n"))
          # Keep as NA
        } else {
        ...
        cat(paste("    Averaging", length(gas_indices), "measurements for", gas, "\n"))
        ...
        for(idx in gas_indices) {
          ...
          if(length(values) == 0) {
            cat(paste("    WARNING: Empty measurement series in", gas, "duplicate", idx, "\n"))
            all_valid <- FALSE
            break
          }
        ...
        if(all_valid && length(all_measurements) > 0) {
          ...
          if(length(unique(lengths)) == 1) {
            ...
            cat(paste("    ERROR: Cannot average", gas, "measurements - different lengths:", paste(lengths, collapse=", "), "\n"))
    ...
    if(length(all_gases_this_site) > length(unique_gases)) {
      ...
      warning(paste("Site", group[[1]]$site_name, "group", group_name, 
                    "missing gases found elsewhere:", paste(missing_gases, collapse=", ")))
    }
